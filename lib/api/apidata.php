...
if ($rpta) {
    if ($estado == 1) {
      ...
      <div class='p-1 pl-2 pr-2'>
        ...
        $rsptax = mysqli_query($Cn, "insert into post (identificador,contenido,creacion,estado) values ('" . $data['IdDispositivo'] . "','" . $dato . "','" . $fecha . "',1)");
    } else {
        $dato = "<div class='d-inline-flex ml-1'>
        <img src='assets/images/music.png' style='width: 30px; height: 30px;' alt='' class='mr-2 mt-1 float-left shadow-sm'>
        <div class='float-left mt-1'>
    ...
    if ($rsptax) {
        ...
        if ($query->num_rows > 0) {
            while ($row = mysqli_fetch_assoc($query)) {
                $postID = $row["idpost"];
                $rata += "<div class='card bg-dark text-white p-1 mt-2 shadow-sm d-flex animable' style='width: 100%;'>" . $row['contenido'] . "</div>";
            }
